            ...
            <div class="form-group">
                ...
                <select class="form-control" id="employee_name" name="employee_name">
                    ...
                    @foreach($users as $user)
                        <option value="{{ $user->name }}" {{ old('employee_name', $request->employee_name) == $user->name ? 'selected' : '' }}>{{ $user->name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
                ...
                <div id="items">
                    @foreach(old('items', isset($request) ? $request->items : []) as $index => $item)
                        <div class="item-group">
                            ...
                            <br/>
                            <button type="button" class="btn btn-secondary add-item">Adicionar Item</button> <button type="button" class="btn btn-danger remove-item">Remover</button>
                            <br/>
                            <br/>
                        </div>
                    @endforeach
